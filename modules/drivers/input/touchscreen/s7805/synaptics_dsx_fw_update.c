...
#define    TP_ID_GPIO_1    48
#define    TP_ID_GPIO_2    49
...
static ssize_t fwu_sysfs_disp_config_block_count_show(struct device *dev,
		struct device_attribute *attr, char *buf);
static ssize_t fwu_sysfs_fwversion_show(struct device *dev,
		struct device_attribute *attr, char *buf);
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (retval < 0) {
		dev_err(rmi4_data->pdev->dev.parent,
				"%s: Failed to read device config ID\n",
				__func__);
		return snprintf(buf, PAGE_SIZE, "err:: Failed to read device config ID\n");
	}
	device_config_id = be_to_uint(config_id);
	dev_info(rmi4_data->pdev->dev.parent,
			"%s: Device config ID = 0x%02x 0x%02x 0x%02x 0x%02x\n",
	...
	memcpy(buf, fwu->read_config_buf, fwu->config_size);
	device_fw_id = rmi4_data->firmware_id;
	return snprintf(buf, PAGE_SIZE, "TP firmware image : %s \nDevice config ID =0x%02x 0x%02x 0x%02x 0x%02x\ndevice_fw_id=%d\n", firmware_name,
			config_id[0],
			config_id[1],
			config_id[2],
			config_id[3],
...
int Get_Tp_ID(void)
{
    ...
    int tp_id_49=0;
    int TP_ID=0;
     gpio_get_value(TP_ID_GPIO_1);
    // printk("gpio_48_v=%d\n",(gpio_get_value(TP_ID_GPIO_1)));
     gpio_get_value(TP_ID_GPIO_2);
     //printk("gpio_49_v=%d\n",(gpio_get_value(TP_ID_GPIO_2)));
     tp_id_48=(gpio_get_value(TP_ID_GPIO_1)>0)? 1:0;
     tp_id_49=(gpio_get_value(TP_ID_GPIO_2)>0)? 1:0;
     TP_ID=(tp_id_48<<1)|tp_id_49;
     printk("TP_ID=%d  tp_id_48=%d  tp_id_49=%d\n",TP_ID,tp_id_48,tp_id_49);
	switch(TP_ID){
               case 1:
                    firmware_name="startup_TPK_update.img";
		   break;
              case 2:
                    firmware_name="startup_Wintek_update.img";
		   break;
              case 3:
                    firmware_name="startup_Token_update.img";
		   break;
              default:
                    firmware_name="Is invalid tp id,can't update firmware";
		   break;
	}
	printk("synaptics firmware_name=%s\n",firmware_name);
	return 0;
 }
