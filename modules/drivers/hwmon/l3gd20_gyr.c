...
*******************************************************************************/
//#define DEBUG 1
#include <linux/module.h>//zfming
//#include <linux/init.h> //zfming
#include <linux/i2c.h>
...
#ifndef L3GD20_GRY_SELF_VERIFY
#define L3GD20_GRY_SELF_VERIFY
#endif
...
#ifdef L3GD20_GRY_SELF_VERIFY
	struct delayed_work selfverify_work;
#endif
};

#ifdef L3GD20_GRY_SELF_VERIFY
#define REG_STATUS_REG		(0x27)	/** STATUS_REG_A register address */
#define ZYXADA_MASK (0x08)
typedef struct {
    int x;
    int y;
    int z;
...
typedef struct {
    ...
    u8 stage1_value;
    u8 stage2_value;
}selfverify_register_config_t;
static const selfverify_register_config_t selfverify_register_config[] = {
    {CTRL_REG1, 0x6F,      0x6F},
    {CTRL_REG2, 0x00,      0x00},
    {CTRL_REG3, 0x00,      0x00},
    {CTRL_REG4, 0xa0,      0xa2},
...
#define SELFVERIFY_REG_COUNT (sizeof(selfverify_register_config)/sizeof(selfverify_register_config_t))
#define selfverify_struct struct l3gd20_data
#define SELFVERIFY_NORMAL_STATUS 0
#define SELFVERIFY_STAGE1_STATUS 1
#define SELFVERIFY_STAGE2_STATUS 2
#define SELFVERIFY_END_STATUS 3

#define SELFVERIFY_FS_RANGE L3GD20_GYR_FS_2000DPS
#define SELFVERIFY_TIMEDLAY 800  //ms

static u8 g_addresses_continuity = 0;
static selfverify_rec_t selfverify_data_rec[SELFVERIFY_END_STATUS];

static u8 selfverify_store_status_before = 0xff;  /* 0: disable; 1:enable; 0xff:initial value */
static int is_selfverify_mode = 0;
static u8 selfverify_status = SELFVERIFY_NORMAL_STATUS; /* 0: no verify; 1: stage 1; 2: after enable self test bit */
static u8 g_selfverify_verified = 0;
static u8 g_selfverify_schwork_init = 0;
static u8 backup_fs_range = L3GD20_GYR_FS_2000DPS;
#endif
			     ...
			     struct l3gd20_triple *data)
{
	...
	x = (s16)(((gyro_out[1]) << 8) | gyro_out[0]);
	y = (s16)(((gyro_out[3]) << 8) | gyro_out[2]);
	z = (s16)(((gyro_out[5]) << 8) | gyro_out[4]);
	switch(gyro->pdata->fs_range) {
	    case L3GD20_GYR_FS_2000DPS:  // sensitivity: 70
	        sensitivity = 70;
	        break;
	    case L3GD20_GYR_FS_500DPS:   // sensitivity: 17.50
	        sensitivity = 17.50;
	        break;
	    case L3GD20_GYR_FS_250DPS:   // sensitivity: 8.75
	    default:
	        sensitivity = 8.75;
	        break;
	}
	hw_d[0] = (s32)(x * sensitivity);
	hw_d[1] = (s32)(y * sensitivity);
	hw_d[2] = (s32)(z * sensitivity);
				...
				if (dev_data->input_poll_dev->
					...
					err = schedule_delayed_work(
						&dev_data->input_poll_dev->work,
							      ...
							      pdata->poll_interval));
#else
					err = schedule_delayed_work(&dev_data->input_poll_dev->work, msecs_to_jiffies(200)); //Need wait for stable output
#endif
...
#ifdef L3GD20_GRY_SELF_VERIFY
static ssize_t attr_get_self_verify_value(struct device *dev,
			       struct device_attribute *attr, char *buf);

static ssize_t attr_set_self_verify_value(struct device *dev,
			       struct device_attribute *attr,
			       const char *buf, size_t size);
...
#endif
#ifdef L3GD20_GRY_SELF_VERIFY
	__ATTR(verifyresult, 0644, attr_get_self_verify_value, attr_set_self_verify_value),
#endif
};
...
#ifdef L3GD20_GRY_SELF_VERIFY
//#define selfverify_i2c_read l3gd20_i2c_read
//#define selfverify_i2c_write l3gd20_i2c_write
//#define selfverify_get_data lis3dh_acc_get_acceleration_data
#define selfverify_is_valid(value) (value <= 800 && value >= 170)
static int selfverify_start (selfverify_struct *stat);
static int selfverify_finish(selfverify_struct *stat);

static int selfverify_i2c_read(selfverify_struct *stat, u8 *buf, int len) {
    if(len > 1) {
        buf[0] = (I2C_AUTO_INCREMENT | buf[0]);
    }
    return l3gd20_i2c_read(stat, buf, len);
}

static int selfverify_i2c_write(selfverify_struct *stat, u8 *buf, int len) {
    if(len > 1) {
        buf[0] = (I2C_AUTO_INCREMENT | buf[0]);
    }
    return l3gd20_i2c_write(stat, buf, len);
}

static int selfverify_get_data(selfverify_struct *stat, int *xyz) {
    struct l3gd20_triple data_out;
    int err = l3gd20_get_data(stat, &data_out);
...
static int selfverify_enable_sensor(selfverify_struct *stat) {
    return l3gd20_enable(stat);
}
static int selfverify_disable_sensor(selfverify_struct *stat) {
    return l3gd20_disable(stat);
}

static void selfverify_close(selfverify_struct *stat) {
    u8 temp_data[2] = {0, 0};

    temp_data[0] = 0x20;
    selfverify_i2c_write(stat, temp_data, 1);
...
static int selfverify_checkresult (void) {
    if(g_selfverify_verified) {
        int data[3];
        
        data[0] = abs((selfverify_data_rec[SELFVERIFY_STAGE2_STATUS].x - selfverify_data_rec[SELFVERIFY_STAGE1_STATUS].x ) / 1000);
        data[1] = abs((selfverify_data_rec[SELFVERIFY_STAGE2_STATUS].y - selfverify_data_rec[SELFVERIFY_STAGE1_STATUS].y )  / 1000);
        data[2] = abs((selfverify_data_rec[SELFVERIFY_STAGE2_STATUS].z - selfverify_data_rec[SELFVERIFY_STAGE1_STATUS].z )  / 1000);
        if(selfverify_is_valid(data[0]) && selfverify_is_valid(data[1]) && selfverify_is_valid(data[2])){
            return 1000;
        }
        return 0;
    }
...
static int selfverify_stage_set_registers(selfverify_struct *stat, int stage) {
    int index;
    int reg_count;
    u8 buf[SELFVERIFY_REG_COUNT+1] = {0};
    int err = -1;

    if(!(stage == SELFVERIFY_STAGE1_STATUS || stage == SELFVERIFY_STAGE2_STATUS)) {
        return -1;
    }
    reg_count = SELFVERIFY_REG_COUNT;
    if(g_addresses_continuity) {
        buf[0] = selfverify_register_config[0].register_address;
        for(index = 0; index < reg_count; index++) {
            if(stage == SELFVERIFY_STAGE1_STATUS) {
                buf[index+1] = selfverify_register_config[index].stage1_value;
            } else {
                buf[index+1] = selfverify_register_config[index].stage2_value;
            }
        }
        err = selfverify_i2c_write(stat, buf, reg_count);
        if (err < 0) {
            pr_info("%s err:%d, stage:%d\n", __func__, err, stage);
            return err;
        }
    } else {
        for(index = 0; index < reg_count; index++) {
            buf[0] = selfverify_register_config[index].register_address;
            if(stage == SELFVERIFY_STAGE1_STATUS) {
                buf[1] = selfverify_register_config[index].stage1_value;
            } else {
                buf[1] = selfverify_register_config[index].stage2_value;
            }

            err = selfverify_i2c_write(stat, buf, 1);
            if (err < 0) {
                pr_info("%s err:%d, stage:%d, addr:0x%x\n", __func__, err, stage, selfverify_register_config[index].register_address);
                return err;
            }
        }
    }
...
static void poll_function_work_selfverify(struct work_struct *work) {
    selfverify_struct *stat;
    int xyz[3] = { 0 };
    int x,y,z;
    int err, index;
    u8 acc_status_data[2] = {0};

    pr_info("[selfverify]%s, stage:%d\n", __func__, selfverify_status);
    stat = container_of((struct delayed_work *)work,
            selfverify_struct, selfverify_work);

    if(selfverify_status == SELFVERIFY_NORMAL_STATUS) {
        selfverify_status = SELFVERIFY_STAGE1_STATUS;
        selfverify_start(stat);
        return;
    }
    mutex_lock(&stat->lock);
    acc_status_data[0] = REG_STATUS_REG;
    err = selfverify_i2c_read(stat, acc_status_data, 1);
    if (err < 0) {
        pr_info("[selfverify]acc_status 1 read, err:%d\n", err);
        goto error_return;
    }
    if(acc_status_data[0] & ZYXADA_MASK) {
        pr_info("[selfverify] 1 ready\n");
    }
    else {
        pr_info("[selfverify] 1 not ready\n");
        schedule_delayed_work(&stat->selfverify_work, msecs_to_jiffies(SELFVERIFY_TIMEDLAY));
        goto error_return;
    }
    err = selfverify_get_data(stat, xyz); /* Need discard */
    if (err < 0) {
        dev_err(&stat->client->dev, "[selfverify]get_accelerometer_data failed\n");
        goto error_return;
    }

    acc_status_data[1] = (acc_status_data[0] & ~ZYXADA_MASK); // for clean ZYXADA bit.
    acc_status_data[0] = REG_STATUS_REG;
    selfverify_i2c_write(stat, acc_status_data, 1);
    memset(xyz, 0, sizeof(xyz));
    x = y = z = 0;
    index = 0;
    while(index < 5) {
        int ready_check_cnt = 0;
        
        /* Start Check the ZYXADA bit */
        do{
            acc_status_data[0] = REG_STATUS_REG;
            err = selfverify_i2c_read(stat, acc_status_data, 1);
            if (err < 0) {
                pr_info("[selfverify]acc_status 2 read, err:%d\n", err);
                break;
            }
            ready_check_cnt ++;
        }while(!(acc_status_data[0] & ZYXADA_MASK) && (ready_check_cnt < 1000));
        if(ready_check_cnt >= 1000) {
            break;
        }
        /* finished Check the ZYXADA bit */
    
        err = selfverify_get_data(stat, xyz);
        if (err < 0) {
            dev_err(&stat->client->dev, "[selfverify]get_accelerometer_data failed\n");
            goto error_return;
        }
        pr_info("[selfverify] index:%d, check:%d read data[%d,%d,%d]\n", index, ready_check_cnt, xyz[0], xyz[1], xyz[2]);
        x += xyz[0];
        y += xyz[1];
        z += xyz[2];
        index++;
    ...
    y = y / index;
    z = z / index;
    selfverify_data_rec[selfverify_status % (SELFVERIFY_END_STATUS)].x =  x;
    selfverify_data_rec[selfverify_status % (SELFVERIFY_END_STATUS)].y =  y;
    selfverify_data_rec[selfverify_status % (SELFVERIFY_END_STATUS)].z =  z;
    pr_info("[selfverify]ready:%d, [%d]:%d, %d, %d\n", 
            index,
            selfverify_status,
            x,
            y,
            z);
    if(selfverify_status == SELFVERIFY_STAGE1_STATUS){
        selfverify_status = SELFVERIFY_STAGE2_STATUS;
        selfverify_stage_set_registers(stat, selfverify_status);
        /* Entry Self test stage */
        schedule_delayed_work(&stat->selfverify_work, msecs_to_jiffies(60));
    }
    else if(selfverify_status == SELFVERIFY_STAGE2_STATUS) {
        selfverify_finish(stat);
    }
    return;
error_return:
    mutex_unlock(&stat->lock);
...
static int selfverify_init (selfverify_struct *stat, u8 enabled) {
    if(selfverify_store_status_before == 0xff) {
        if(enabled) {
            selfverify_store_status_before = 1;
        } else {
            selfverify_store_status_before = 0;
        }

        if(!g_selfverify_schwork_init) {
            INIT_DELAYED_WORK(&stat->selfverify_work, poll_function_work_selfverify);
            g_selfverify_schwork_init = 1;
        }
        if(is_selfverify_mode && selfverify_status == SELFVERIFY_NORMAL_STATUS) {
            if(selfverify_store_status_before == 1)
                selfverify_disable_sensor(stat);
            schedule_delayed_work(&stat->selfverify_work, msecs_to_jiffies(10));
        }
    }
...
static ssize_t attr_get_self_verify_value(struct device *dev,
			       struct device_attribute *attr, char *buf)
{
    pr_info("%s, result:%d\n",__func__, selfverify_checkresult());
    return sprintf(buf, "%d\n", selfverify_checkresult());
}
...
static ssize_t attr_set_self_verify_value(struct device *dev,
			       ...
			       const char *buf, size_t size)
{
    selfverify_struct *stat = dev_get_drvdata(dev);
    unsigned long val;
    ...
    is_selfverify_mode = val;
    if(atomic_read(&stat->enabled)) {
        selfverify_init(stat, 1);
    } else {
        selfverify_init(stat, 0);
    }
    pr_info("%s, value:%d\n",__func__, is_selfverify_mode);
    return size;
}

static void selfverify_check_registers_continuity(void) {
    int index;

    for(index=1; index<SELFVERIFY_REG_COUNT; index++) {
        if(selfverify_register_config[index].register_address != selfverify_register_config[index-1].register_address + 1) {
            g_addresses_continuity = 0;
            return;
        }
    }
    g_addresses_continuity = 1;
...
static int selfverify_start (selfverify_struct *stat) {
    ...
    selfverify_check_registers_continuity();
    selfverify_stage_set_registers(stat, selfverify_status);
    backup_fs_range = stat->pdata->fs_range;
    stat->pdata->fs_range = SELFVERIFY_FS_RANGE;
    schedule_delayed_work(&stat->selfverify_work, msecs_to_jiffies(SELFVERIFY_TIMEDLAY));
    g_selfverify_verified = 0;
...
static int selfverify_finish(selfverify_struct *stat) {
    ...
    selfverify_status = SELFVERIFY_NORMAL_STATUS;
    is_selfverify_mode = 0;
    g_selfverify_verified = 1;
    for(index=0; index<3; index++) {
        pr_info("[selfverify]result [%d] %d, %d, %d\n", index, selfverify_data_rec[index].x, selfverify_data_rec[index].y, selfverify_data_rec[index].z);
    }
    selfverify_close(stat);
    stat->pdata->fs_range = backup_fs_range;
    /* Resume normal stage */
    if(selfverify_store_status_before) {
        selfverify_enable_sensor(stat);
    }
    selfverify_store_status_before = 0xff;
					...
					const struct i2c_device_id *devid)
{
	...
	if (client->dev.platform_data == NULL) {
		//int1_gpio = get_gpio_by_name("gyro_int");
		//int2_gpio = get_gpio_by_name("gyro_drdy");
	...
	pr_info("%s: gpio_int1:%d, gpio_int2:%d, \n", __func__, gyro->pdata->gpio_int1, gyro->pdata->gpio_int2);
	...
	if(!intel_scu_ipc_read_msic_vprog2_reg()) {
		...
		printk("%s, enable vprog2 to L3gd20_gyr\n",__func__);
		msleep(1);
	}
	...
	if(gyro->pdata->gpio_int1 < 0)
           gyro->pdata->gpio_int1 = get_gpio_by_name("gyro_int");

	if(gyro->pdata->gpio_int2 < 0)
           gyro->pdata->gpio_int2 = get_gpio_by_name("gyro_drdy");
