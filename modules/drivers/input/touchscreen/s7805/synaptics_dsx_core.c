...
static unsigned char  max_point=0;
...
static void synaptics_rmi4_set_params(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (f1a) {
        ...
        max_point=rmi4_data->num_of_fingers;
	return;
}
...
static ssize_t synaptics_point_show(struct device *dev,
...
static ssize_t synaptics_point_store(struct device *dev,
					struct device_attribute *attr,
...
static DEVICE_ATTR(point, (S_IWUSR|S_IRUGO), synaptics_point_show,
			synaptics_point_store);
...
static int synaptics_touch_sysfs_init(void)
{
	...
	ret = sysfs_create_file(android_touch_ftobj, &dev_attr_point.attr);
	if (ret) 
	{
		printk(KERN_ERR "TOUCH_ERR: create_file point failed\n");
		return ret;
	}
return 0;
}
