...
//#define	DEBUG		1
#ifndef ACCEL_SELF_VERIFY
#define ACCEL_SELF_VERIFY
#endif
...
#define selfverify_struct struct lis3dh_acc_status
...
#define SELFVERIFY_FS_RANGE LIS3DH_ACC_G_2G
#define SELFVERIFY_TIMEDLAY 200  //ms
...
static u8 backup_fs_range = LIS3DH_ACC_G_2G;
#endif
...
#endif
#ifdef ACCEL_SELF_VERIFY
	__ATTR(verifyresult, 0664, attr_get_self_verify_value, attr_set_self_verify_value),
#endif
};
...
#ifdef ACCEL_SELF_VERIFY
#define selfverify_i2c_read lis3dh_acc_i2c_read
#define selfverify_i2c_write lis3dh_acc_i2c_write
#define selfverify_get_data lis3dh_acc_get_acceleration_data
#define selfverify_enable_sensor lis3dh_acc_enable
#define selfverify_disable_sensor lis3dh_acc_disable

#define selfverify_is_valid(value) (value <= 1700 && value >= 60)
...
static int selfverify_checkresult (void) {
    if(g_selfverify_verified) {
        ...
        data[0] = abs(selfverify_data_rec[SELFVERIFY_STAGE2_STATUS].x - selfverify_data_rec[SELFVERIFY_STAGE1_STATUS].x );
        data[1] = abs(selfverify_data_rec[SELFVERIFY_STAGE2_STATUS].y - selfverify_data_rec[SELFVERIFY_STAGE1_STATUS].y );
        data[2] = abs(selfverify_data_rec[SELFVERIFY_STAGE2_STATUS].z - selfverify_data_rec[SELFVERIFY_STAGE1_STATUS].z );
		...
		const struct i2c_device_id *id)
{
	...
	if(!intel_scu_ipc_read_msic_vprog2_reg()) {
		...
		printk("%s, enable vprog2 to lis3dh_acc\n",__func__);
		msleep(1);
	}
...
static int __exit lis3dh_acc_remove(struct i2c_client *client)
{
...
//Change module_init as late_initcall, Because the sensor loading too earlier , 
//The sensor visit I2C would bring controller timeout sometimes. this changes refer to intel bugzilla 621.
