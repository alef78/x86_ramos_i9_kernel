...
#define SELFVERIFY_FS_RANGE LSM303D_ACC_FS_2G
#define SELFVERIFY_TIMEDLAY 100  //ms
...
static u8 backup_fs_range = LSM303D_ACC_FS_2G;
#endif
...
#ifdef ACCEL_SELF_VERIFY
static ssize_t attr_get_self_verify_value(struct kobject *kobj,
					struct kobj_attribute *attr,
					char *buf);

static ssize_t attr_set_self_verify_value(struct kobject *kobj,
					struct kobj_attribute *attr,
					const char *buf, size_t size);

static struct kobj_attribute fs_attr_verifyresult = 
...
#ifdef ACCEL_SELF_VERIFY
#define selfverify_i2c_read lsm303d_i2c_read
#define selfverify_i2c_write lsm303d_i2c_write
#define selfverify_get_data lsm303d_acc_get_data
#define selfverify_enable_sensor lsm303d_acc_enable
#define selfverify_disable_sensor lsm303d_acc_disable
...
static int selfverify_checkresult (void) {
    if(g_selfverify_verified) {
        ...
        data[0] = abs(selfverify_data_rec[SELFVERIFY_STAGE2_STATUS].x / 1000 - selfverify_data_rec[SELFVERIFY_STAGE1_STATUS].x / 1000);
        data[1] = abs(selfverify_data_rec[SELFVERIFY_STAGE2_STATUS].y / 1000 - selfverify_data_rec[SELFVERIFY_STAGE1_STATUS].y / 1000);
        data[2] = abs(selfverify_data_rec[SELFVERIFY_STAGE2_STATUS].z / 1000 - selfverify_data_rec[SELFVERIFY_STAGE1_STATUS].z / 1000);
...
static int selfverify_start (selfverify_struct *stat) {
    ...
    selfverify_stage_set_registers(stat, selfverify_status);
    backup_fs_range = stat->pdata_acc->fs_range;
    stat->pdata_acc->fs_range = SELFVERIFY_FS_RANGE;
    schedule_delayed_work(&stat->selfverify_work, msecs_to_jiffies(SELFVERIFY_TIMEDLAY));
...
static int selfverify_finish(selfverify_struct *stat) {
    ...
    stat->pdata_acc->fs_range = backup_fs_range;
    /* Resume normal stage */
    if(selfverify_store_status_before == 1) {
		...
		const struct i2c_device_id *id)
{
	...
	stat->pdata_acc->gpio_int2 = get_gpio_by_name("accel_int2"); //zfming
	...
	if(!intel_scu_ipc_read_msic_vprog2_reg()) {
		...
		printk("%s, enable vprog2 to Lsm303d\n",__func__);
		msleep(1);
	}
