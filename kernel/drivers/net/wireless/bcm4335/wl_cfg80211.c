 ...
 */
/* ToDo; To be modified according to regulatory dynamically loaded !!! (In order to inform the kernel...)
 * This table is provided to the kernel where it determines channels flags.
 * So it is required for 4339 and 4354 to add 80MHz bandwidth for 5GHz channels.
 */
...
s32 wl_update_wiphybands(struct wl_priv *wl, bool notify)
{
	...
	err = wl_construct_reginfo(wl, mimo_bw_cap);
	if (err) {
